...
def getJsonData():
    ...
    try:
        txid_file = open("last_txid",'r')
    except FileNotFoundError:
        last_txid_file = open("last_txid", 'w+')
        last_txid_file.close()
        txid_file = open("last_txid",'r')
    #print(data)
    tx_store=[]
    for txid in txid_file:
        tx_store.append(int(txid.strip()))
    Dapps = []
    length = len(tx_store)
    i = 0
    while (i < length):
        content = readUnitFromBlockchain(data['last_txs'][tx_store[i]]['addresses'])
        pos = content.find('{')
        app = json.loads(content[pos:])
        search_pos = searchDict(Dapps, 'id', app['id'])
        if (search_pos != -1):
            del (Dapps[search_pos])
        if ('remove' not in app.keys()):
            Dapps = Dapps + [app]
        i = i + 1
    if(length > 0):
        i = tx_store[length-1] + 1
    else:
        i = 0
    while i < (len(data['last_txs'])):
        if(data['last_txs'][i]['type']=='vin'):
            ...
            if content.startswith("text:Dapps"):
                ...
                last_txid_file = open("last_txid",'a')
                last_txid_file.write(str(i)+'\n')
                last_txid_file.close()
                pos = content.find('{')
                app = json.loads(content[pos:])
                search_pos = searchDict(Dapps,'id',app['id'])
